...
class OrderRepository:
    ...
    def create_order(self, user_id: int, slot_id: int, total_price: float, notes: str, items_data: list) -> Order:
        ...
        for item in items_data:
            self.db.add(OrderItem(order_id=order.id, dish_id=item["dish_id"], quantity=item["quantity"], price=item["price"]))
    ...
    def get_order(self, order_id: int) -> Optional[Order]:
        return self.db.query(Order).options(
            ...
            joinedload(Order.slot), joinedload(Order.user)
        ).filter(Order.id == order_id).first()

    def get_user_orders(self, user_id: int) -> list[Order]:
        return self.db.query(Order).options(
            joinedload(Order.items).joinedload(OrderItem.dish), joinedload(Order.slot)
