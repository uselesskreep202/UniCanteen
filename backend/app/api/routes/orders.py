...
def create_order(data: OrderCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return _serialize(OrderService(db).create_order(user.id, data))

@router.get("/my", response_model=list[OrderOut])
def my_orders(db: Session = Depends(get_db), user=Depends(get_current_user)):
    return [_serialize(o) for o in OrderRepository(db).get_user_orders(user.id)]
...
def all_orders(status: Optional[str] = None, db: Session = Depends(get_db), _=Depends(require_staff)):
    ...
    return [_serialize(o) for o in OrderRepository(db).get_all_orders(status=st)]
...
def update_status(order_id: int, data: OrderStatusUpdate, db: Session = Depends(get_db), _=Depends(require_staff)):
    ...
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    return _serialize(repo.update_status(order, data.status))
...
def cancel_order(order_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    if order.status != OrderStatus.PENDING:
        raise HTTPException(status_code=400, detail="Можно отменить только заказ в статусе 'ожидает'")
    return _serialize(repo.update_status(order, OrderStatus.CANCELLED))

def _serialize(order):
    result = OrderOut.model_validate(order)
    for i, item in enumerate(result.items):
        if order.items and i < len(order.items) and order.items[i].dish:
            item.dish_name = order.items[i].dish.name
    return result