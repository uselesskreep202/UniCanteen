...
export const getSlots = (date?: string) =>
  api.get<TimeSlot[]>("/orders/slots", { params: date ? { date } : {} });

export const createSlot = (data: Omit<TimeSlot, "id" | "current_orders">) =>
  api.post<TimeSlot>("/orders/slots", data);
...
export const getAllOrders = (status?: string) =>
  api.get<Order[]>("/orders/all", { params: status ? { status } : {} });
...
export const getStats = () =>
  api.get<Record<string, number>>("/orders/stats/summary");
